...
\textbf{Description:} A comprehensive real-world dataset containing incident reports of crimes occurred in the City of Chicago. We have fetched a subset of 10,000 recent records via the official API to satisfy the project constraints. The key attributes include:
\begin{itemize}
    \item \texttt{id}, \texttt{case\_number}, \texttt{date}: Incident identifiers and timestamp.
    \item \texttt{primary\_type}, \texttt{description}: The type and specific description of the crime.
    \item \texttt{location\_description}: Where the incident took place (e.g., STREET, APARTMENT).
    \item \texttt{arrest}, \texttt{domestic}: Boolean flags indicating if an arrest was made or if it was domestic-related.
    \item \texttt{district}, \texttt{ward}, \texttt{community\_area}: Geographical boundaries.
\end{itemize}
...
Below is the Java MapReduce code containing both Mapper and Reducer classes:
...
\section{Reducer Logic Explanation}
...
\newpage
\section{Hive Analysis}
The following 10 meaningful queries were executed in Apache Hive on the dataset to extract business intelligence.

\subsection{Hive Analysis Questions and Queries}
...
\subsection{Screenshot of Query Results}
\begin{figure}[h]
    \centering

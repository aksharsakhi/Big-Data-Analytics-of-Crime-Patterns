...
\section{Phase 7: Apache Hive Setup \& Individual Queries (Copy-Paste Ready)}
Executed directly on VM Terminal:
\begin{lstlisting}[language=bash]
...
\subsection{Hive Database DDL \& Table Setup Commands}

\textbf{Set Default Database:}
\begin{lstlisting}[language=SQL]
USE default;
\end{lstlisting}

\textbf{Create crimes Table:}
\begin{lstlisting}[language=SQL]
CREATE TABLE crimes (
...
cd ..
git add .
git commit -m "Update commands_executed documentation with separate code blocks for each query"
git push
\end{lstlisting}

\section{Quick Reference: Core MapReduce \& Hive Commands}
Below are the two primary commands executed to demonstrate MapReduce and Hive Query functionality:

\subsection{1. MapReduce Java Application Execution}
\begin{lstlisting}[language=bash]
hadoop jar crimecount.jar bigdata.CrimeTypeCount /user/hadoop/bigdata_project/crimes /user/hadoop/bigdata_project/crime_output
\end{lstlisting}

\subsection{2. Apache Hive Analytical Query Execution}
\begin{lstlisting}[language=SQL]
SELECT location_description, COUNT(*) AS crime_count FROM crimes GROUP BY location_description ORDER BY crime_count DESC LIMIT 15;
\end{lstlisting}
